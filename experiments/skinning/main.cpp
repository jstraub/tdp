...
std::vector<tdp::Vector3fda> getMeanAndSpread(const tdp::ManagedHostImage<tdp::Vector3fda>& pc){
    ...
    for (int i=0; i< cov.rows();++i ){
      if (maxEval < std::real(es.eigenvalues().col(0)[i])){
          maxEval = std::real(es.eigenvalues().col(0)[i]);
          std::cout << "curr eval: " << es.eigenvalues().col(0)[i] << std::endl;
          maxIdx = i;
      }
    }
    ...
    spec.push_back(mean);
    spec.push_back(spread_real);
    spec.push_back(spread_imag);
    return spec;
}
...
std::vector<tdp::Vector3fda> meanAndSpreadOfBVoxel(const tdp::ManagedHostImage<tdp::Vector3fda>& pc, const tdp::Vector3fda& p1, const tdp::Vector3fda& p2){
    ...
    std::cout << "meanAndSpreadofbvoxel called..." <<std::endl;
    ...
    int count = 0;
    tdp::ManagedHostImage<tdp::Vector3fda> inPoints;
    for (int i=0; i<pc.w_; ++i){
        for (int j=0; j<pc.h_; ++j){
            if (inBVoxel(pc(i,j), topLeft, btmRight)){
                inPoints(0,count) = pc(i,j);
                count += 1;
            }
        }
    }
    tdp::Vector3fda mean = getMean(inPoints);
    tdp::Matrix3fda cov = getCovariance(inPoints);
    std::cout << "mean of this bvoxel: \n" << mean << std::endl;
    std::cout << "covar of this bvoxel: \n" << cov << std::endl;
    ...
    // get largest eigenvalue's eigenvector
    float maxEval = -1;
    int maxIdx = -1;
    for (int i=0; i< cov.rows();++i ){
        if (maxEval < std::real(es.eigenvalues().col(0)[i])){
