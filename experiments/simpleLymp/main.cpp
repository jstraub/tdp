...
int main( int argc, char* argv[] )
{
  ...
  const float tsdfMu = .002;
  const float tsdfWThr = 3;
  const float tsdfWMax = 100;

  const float marchCubesfThr = 1.;
  const float marchCubeswThr = 0;

  const float grid0x = -.175;
  const float grid0y = -.116;
  const float grid0z = .32;
  const float gridEx = .18;
  const float gridEy = .074;
  const float gridEz = .5;

  const float icpAngleThr_deg = 15.;
  const float icpDistThr = .03;
  const int icpIter0 = 10;
  const int icpIter1 = 7;
  const int icpIter2 = 5;

  const int ir = 16; // ir power
  const int rotatingDepthScanIrPower = ir;
  const int stabilizationTime = 30;
...
/*
 * THAT WAS IT FOLKS, THE REST IS MORE GUI STUFF AGAIN
 */
