...
void ImuOutStream::Open(const std::string& uri,
  ...
  if (packetstreamsrcid == -1) {
    ...
    pangolin::PacketStreamSource pss = packetstream.CreateSource(
        "imu_obs", input_uri, json_header, total_frame_size,
        "struct ImuObs{"
...
int ImuOutStream::WriteStream(const ImuObs& data,
                              const picojson::value& frame_properties) {
#ifndef _WIN_
  if (is_pipe) {
    ...
    if (!packetstream.IsOpen()) {
      ...
      if (fd != -1) {
        ...
        if (errno == ENXIO) {
          ...
          pangolin::SigState::I().sig_callbacks.at(SIGPIPE).value = false;
  ...
  packetstream.WriteSourcePacket(packetstreamsrcid, (char*)&data,
                                 total_frame_size);
