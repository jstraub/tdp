...
void KernelRayTraceTSDF(Volume<float> tsdf, Image<float> d, 
  ...
  if (idx < d.w && idy < d.h) {
    ...
    for (size_t id=tsdf.d_; id>0; --id) {
      ...
      float tsdfVal = tsdf(floor(u_r(0)+0.5), floor(u_r(1)+0.5), id);
...
void KernelAddToTSDF(Volume<float> tsdf, Volume<float> W, Image<float> d, 
  ...
  if (idx < tsdf.w && idy < tsdf.h && idz < tsdf.d) {
    float rho_r = rho0+drho*idz;
    Eigen::Vector3f p_r = camR.Unproject(idx,idy,1./rho_r);
    Eigen::Vector2f u_d = camD.Project(T_rd.inverse()*p_r);
    float z_d = d(floor(u_d(0)+0.5), floor(u_d(1)+0.5));
    float lambda = camD.Unproject(u_d(0),u(1),1.).norm();
    float z_tsdf = (T_rd.translation()-p_r).norm()/lambda;

    float eta = z_tsdf - z_d;
    float psi = eta>=-mu? min(1.,eta/mu)*sign(eta);
...
void AddToTSDF(Volume<float> tsdf, Volume<float> W, Image<float> d, 
  ...
  KernelDepthConvert<<<blocks,threads>>>(tsdf, W, d, T_rd, camR, camD, rho0, drho, mu);
}
