...
Eigen::Quaternion<float> NodeLin::GetCenter() const {
  return Project(nodeLin_.GetBox().GetCenter());
}

Eigen::Vector4f NodeLin::QuaternionToVec(const Eigen::Quaternion<float>& q) {
...
float NodeLin::GetVolume_() const { 
  // subdivide box in Lin space into 4 tetrahedra and sum their volumes
  // https://www.ics.uci.edu/~eppstein/projects/tetra/
  // NodeS3 1: 0 4 5 7
  float V = TetraFromBox(nodeLin_.GetBox(), 0, 4, 5, 7).GetVolume();
  // NodeS3 2: 1 4 5 6
