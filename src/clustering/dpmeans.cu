...
__global__ void KernelDpMeansLabelAssign(
    Image<Vector3fda> x,
    Image<Vector3fda> mu,
    Image<uint16_t> z,
  ...
  for(int id=i0+idx*N_PER_T; id<min((int)x.Area(),(int)((idx+1)*N_PER_T)); ++id) {
    uint16_t z_i = K;
    ...
    float lambda, uint32_t i0, uint16_t K)
{
  ...
  dim3 threads, blocks;
  ComputeKernelParamsForArray(blocks,threads,(x.Area()-i0)/10,256);
  KernelDpMeansLabelAssign<256><<<blocks,threads>>>(
      x, mu, z, lambda, IidAction.ptr_, i0, K, 10); 
