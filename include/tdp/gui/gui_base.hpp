 ...
 public:
  ...
  void SeekFrames(int destFrame) {
    if (video_playback && destFrame < end_frame) {
      std::cout << "SeekFrames seek " << video_playback->Seek(destFrame)
                << std::endl;
      finished_ = false;
      //      GrabFrames();
  ...
  bool ImageD(tdp::Image<uint16_t>& d, size_t camId = 0,
    ...
    d = tdp::Image<uint16_t>(
        images[iD[camId]].w, images[iD[camId]].h, images[iD[camId]].pitch,
        reinterpret_cast<uint16_t*>(images[iD[camId]].ptr), Storage::Cpu);
  ...
  bool ImageRGB(tdp::Image<tdp::Vector3bda>& rgb, size_t camId = 0,
    ...
    rgb = tdp::Image<tdp::Vector3bda>(
        images[iRGB[camId]].w, images[iRGB[camId]].h, images[iRGB[camId]].pitch,
  ...
  pangolin::View& container() { return pangolin::Display("container"); }
 ...
 protected:
  ...
  bool GrabFrames() {
    if (video.Grab(&buffer[0], images, video_wait, video_newest)) {
      picojson::value props = pangolin::GetVideoFrameProperties(&video);
      //        std::cout << props.serialize(true) << std::endl;
      t_host_us_.clear();
      if (props.contains("streams")) {
        for (size_t i = 0; i < images.size(); ++i) {
          if (props["streams"][i].contains("hosttime_us")) {
            t_host_us_.push_back(
                props["streams"][i]["hosttime_us"].get<int64_t>());
          }
        }
      } else {
        //        std::cout << "could not find strem properties in: " <<
        //        std::endl
