...
struct Rig {
  ...
  bool ParseTransformation(const picojson::value& jsT, SE3f& T) {
    picojson::value t_json = jsT["t_xyz"];
    Eigen::Vector3f t(t_json[0].get<double>(), t_json[1].get<double>(),
                      t_json[2].get<double>());
    Eigen::Matrix3f R;
    if (jsT.contains("q_wxyz")) {
      picojson::value q_json = jsT["q_wxyz"];
      Eigen::Quaternionf q(q_json[0].get<double>(), q_json[1].get<double>(),
                           q_json[2].get<double>(), q_json[3].get<double>());
      R = q.toRotationMatrix();
    } else if (jsT.contains("R_3x3")) {
      picojson::value R_json = jsT["R_3x3"];
      R << R_json[0].get<double>(), R_json[1].get<double>(),
          R_json[2].get<double>(), R_json[3].get<double>(),
          R_json[4].get<double>(), R_json[5].get<double>(),
  ...
  bool FromFile(std::string pathToConfig, bool verbose) {
    ...
    if (f.is_open()) {
      std::string err = picojson::parse(file_json, f);
      if (!err.empty()) {
          ...
          for (size_t i = 0; i < file_json.size(); ++i) {
            if (file_json[i].contains("camera")) {
              ...
              if (file_json[i]["camera"].contains("serialNumber")) {
                ...
                if (verbose)
                  std::cout << "Serial ID: " << serials_.back() << std::endl;
              }
              Cam cam;
              if (cam.FromJson(file_json[i]["camera"])) {
                if (verbose) std::cout << "found camera model" << std::endl;
                //                std::cout <<
                //                file_json[i]["camera"].serialize(true) <<
                //                std::endl;
              }
              cams_.push_back(cam);
              if (file_json[i]["camera"].contains("depthScale")) {
                std::string path =
                    CONFIG_DIR +
                    file_json[i]["camera"]["depthScale"].get<std::string>();
              ...
              std::cout << "found IMU: "
                        << file_json[i]["imu"]["type"].get<std::string>()
                        << std::endl;
  ...
  bool ToFile(std::string pathToConfig, bool verbose) { return false; }

  bool CorrespondOpenniStreams2Cams(
      const std::vector<pangolin::VideoInterface*>& streams, int maxDevs = 999);

  void CollectRGB(const GuiBase& gui, Image<Vector3bda>& rgb);

  void CollectD(const GuiBase& gui, float dMin, float dMax,
                Image<uint16_t>& cuDraw, Image<float>& cuD,
                int64_t& t_host_us_d);
  ...
  void AddToTSDF(const Image<float>& cuD, const SE3f& T_mr,
                 bool useRgbCamParasForDepth, const Vector3fda& grid0,
                 const Vector3fda& dGrid, float tsdfMu, float tsdfWMax,
                 Volume<TSDFval>& cuTSDF);

  void AddToTSDF(const Image<float>& cuD, const Image<Vector3bda>& cuRgb,
                 const SE3f& T_mr, bool useRgbCamParasForDepth,
                 const Vector3fda& grid0, const Vector3fda& dGrid, float tsdfMu,
                 float tsdfWMax, Volume<TSDFval>& cuTSDF);

  void AddToTSDF(const Image<float>& cuD, const Image<Vector3bda>& cuRgb,
                 const Image<Vector3fda>& cuN, const SE3f& T_mr,
                 bool useRgbCamParasForDepth, const Vector3fda& grid0,
                 const Vector3fda& dGrid, float tsdfMu, float tsdfWMax,
                 Volume<TSDFval>& cuTSDF);

  template <int LEVELS>
  void RayTraceTSDF(const Volume<TSDFval>& cuTSDF, const SE3f& T_mr,
                    bool useRgbCamParasForDepth, const Vector3fda& grid0,
                    const Vector3fda& dGrid, float tsdfMu, float tsdfWThr,
                    Pyramid<Vector3fda, LEVELS>& cuPyrPc,
                    Pyramid<Vector3fda, LEVELS>& cuPyrN);
  ...
  Image<T> GetStreamRoi(const Image<T>& I, size_t streamId,
                        float scale = 1.) const {
    size_t camMin =
        *std::min_element(rgbdStream2cam_.begin(), rgbdStream2cam_.end());
...
bool Rig<CamT>::CorrespondOpenniStreams2Cams(
    const std::vector<pangolin::VideoInterface*>& streams, int maxDevs) {
  rgbStream2cam_.clear();
  dStream2cam_.clear();
  rgbdStream2cam_.clear();

  picojson::value devProps = pangolin::GetVideoDeviceProperties(streams[0]);
  ...
  if (devProps.contains("openni")) {
    ...
    std::cout << "cannot match based on serial number because no openni or "
                 "realsense streams; have "
  ...
  picojson::value jsDevices = devProps[devType]["devices"];
  ...
  for (size_t i = 0; i < jsDevices.size() && i < maxDevs; ++i) {
    ...
    if (camId < 0) {
      ...
      std::cout
          << "matching camera in config: " << camId << " "
  ...
  std::cout << "Found " << NumStreams() << " stream paired them with "
            << NumCams() << " cams" << std::endl;
  return true;
}

template <class CamT>
void Rig<CamT>::CollectRGB(const GuiBase& gui, Image<Vector3bda>& rgb) {
...
template <class CamT>
void Rig<CamT>::CollectD(const GuiBase& gui, float dMin, float dMax,
                         Image<uint16_t>& cuDraw, Image<float>& cuD,
                         int64_t& t_host_us_d) {
  ...
  for (size_t sId = 0; sId < rgbdStream2cam_.size(); sId++) {
    ...
    if (cuDepthScales_.size() > cId) {
      ...
      tdp::ConvertDepthGpu(cuDraw_i, cuD_i, cuDepthScales_[cId], a, b, dMin,
                           dMax);
    } else if (depthSensorUniformScale_.size() > cId) {
      tdp::ConvertDepthGpu(cuDraw_i, cuD_i, depthSensorUniformScale_[cId], dMin,
                           dMax);
    } else {
...
template <class CamT>
void Rig<CamT>::ComputeNormals(Image<float>& cuD, bool useRgbCamParasForDepth,
                               Image<Vector3fda>& cuN) {
...
template <class CamT>
void Rig<CamT>::ComputePc(Image<float>& cuD, bool useRgbCamParasForDepth,
                          Image<Vector3fda>& cuPc) {
  for (size_t sId = 0; sId < dStream2cam_.size(); sId++) {
    int32_t cId;
    if (useRgbCamParasForDepth) {
...
template <class CamT>
void Rig<CamT>::AddToTSDF(const Image<float>& cuD, const SE3f& T_mr,
                          bool useRgbCamParasForDepth, const Vector3fda& grid0,
                          const Vector3fda& dGrid, float tsdfMu, float tsdfWMax,
                          Volume<TSDFval>& cuTSDF) {
  for (size_t sId = 0; sId < dStream2cam_.size(); sId++) {
...
void Rig<CamT>::AddToTSDF(const Image<float>& cuD,
                          const Image<Vector3bda>& cuRgb, const SE3f& T_mr,
                          bool useRgbCamParasForDepth, const Vector3fda& grid0,
                          const Vector3fda& dGrid, float tsdfMu, float tsdfWMax,
                          Volume<TSDFval>& cuTSDF) {
  for (size_t sId = 0; sId < dStream2cam_.size(); sId++) {
    ...
    //    (wSingle, hSingle, cuD.ptr_+rgbdStream2cam_[sId]*wSingle*hSingle);
    TSDF::AddToTSDF<CamT::NumParams, CamT>(cuTSDF, cuD_i, cuRgb_i, T_mo, cam,
                                           grid0, dGrid, tsdfMu, tsdfWMax);
  }
}
...
template <class CamT>
template <int LEVELS>
void Rig<CamT>::RayTraceTSDF(const Volume<TSDFval>& cuTSDF, const SE3f& T_mr,
  ...
  for (size_t sId = 0; sId < dStream2cam_.size(); sId++) {
    ...
    tdp::TSDF::RayTraceTSDF<CamT::NumParams, CamT>(
        cuTSDF, cuPcEst_i, cuNEst_i, T_mo, cam, grid0, dGrid, tsdfMu, tsdfWThr);
  }
...
#ifndef __CUDACC__
template <class CamT>
void Rig<CamT>::Render3D(const SE3f& T_mr, float scale) {
  for (size_t sId = 0; sId < dStream2cam_.size(); sId++) {
    int32_t cId;
    //    if (useRgbCamParasForDepth) {
    cId = rgbStream2cam_[sId];
    ...
    pangolin::glDrawFrustrum(cam.GetKinv(), wSingle, hSingle, T_mo.matrix(),
                             scale);
