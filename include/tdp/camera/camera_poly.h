...
class CameraPoly3 : public CameraBase<T, 7, CameraPoly3<T>> {
 public:
  ...
  Eigen::Matrix<T, 2, 1> Project(const Eigen::Matrix<T, 3, 1>& p) const {
    ...
    T r2 = r * r;
    T r4 = r2 * r2;
    T scale = 1 + r2 * this->params_(4) + r4 * this->params_(5) +
              r4 * r2 * this->params_(6);
  ...
  Eigen::Matrix<T, 3, 1> Unproject(T u, T v, T z) const {
    Eigen::Matrix<T, 2, 1> ray((u - this->params_(2)) / this->params_(0),
    ...
    for (int i = 0; i < 5; i++) {
      ...
      // 2nd derivative
      T d2 =
          (4 * ru * (ru * pol - r) * (3 * k1 + 10 * k2 * ru2 + 21 * k3 * ru4) +
           2 * pol3 * pol3);
      // Delta update
      T delta = d / d2;
      ru -= delta;
  ...
  Eigen::Matrix<T, 2, 3> Jproject(const Eigen::Matrix<T, 3, 1>& p) const {
    ...
    Jpi << this->params_(0) / p(2), 0.,
        -p(0) * this->params_(0) / (p(2) * p(2)), 0., this->params_(1) / p(2),
        -p(1) * this->params_(1) / (p(2) * p(2));
    return Jpi;
  }

  bool FromJson(picojson::value& val) {
    if (!val.contains("model")) return false;
    if (!val["model"].contains("type")) return false;
  ...
  CameraPoly3<T> Scale(T scale) const {
    Eigen::Matrix<T, 7, 1> paramsScaled = this->params_;
...
typedef CameraBase<float, CameraPoly3<float>::NumParams, CameraPoly3<float>>
    BaseCameraPoly3f;
